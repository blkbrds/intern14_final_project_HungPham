...
final class MusicViewController: UIViewController {
    ...
    func configUI() {
        title = App.String.musicTitle
        navigationController?.navigationBar.backgroundColor = .red

        collectionView.register(UINib(nibName: "ChannelVideosCell", bundle: nil), forCellWithReuseIdentifier: "musicVideo")
...
extension MusicViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "musicVideo", for: indexPath) as? ChannelVideosCell else { return UICollectionViewCell() }
        let myChannels = viewModel.getMusicVideos(with: indexPath)
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if indexPath.row == viewModel.musicVideos.count - 4 {
